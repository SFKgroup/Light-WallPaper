...
class Window(QWidget):
    ...
    def __init__(self):
        ...
        # 加载字体
        font = QFontDatabase.applicationFontFamilies(
            QFontDatabase.addApplicationFont(setting['font']))
        if font:
            self.fontFamily = font[0]
        else:
            self.fontFamily = None

        # 选择框
        self.listwidget = QListWidget()
        for item in setting["page_dic"].keys():
            self.listwidget.addItem(item)
        self.listwidget.clicked.connect(self.list_clicked)
        self.listwidget.setFont(
            QFont(self.fontFamily, setting['font_size'], QFont.Black))
        self.listwidget.setStyleSheet(
            'QListWidget{color:#EEE;background-color:#313131;padding: 7px 15px;}')
        ...
        self.button_add.setFont(
            QFont(self.fontFamily, setting['font_size'], QFont.Black))
        self.button_add.setStyleSheet(
            'QPushButton{color:#EEE;background-color:#313131;padding: 7px 15px;}QPushButton:hover{background:#111;color:'+setting["theme_colour"]+'}')
        ...
        self.button_set.setFont(
            QFont(self.fontFamily, setting['font_size'], QFont.Black))
        self.button_set.setStyleSheet(
            'QPushButton{color:#EEE;background-color:#313131;padding: 7px 15px;}QPushButton:hover{background:#111;color:'+setting["theme_colour"]+'}')
        ...
        self.button_del.setFont(
            QFont(self.fontFamily, setting['font_size'], QFont.Black))
        self.button_del.setStyleSheet(
            'QPushButton{color:#EEE;background-color:#412121;padding: 7px 15px;}QPushButton:hover{background:#111;color:'+setting["theme_colour"]+'}')
        ...
        if not setting['block_home']:
            action_show = QAction(qtawesome.icon(
                'fa.home', color='white'), f'{language["home"]}', self, triggered=self.show)
            self.icon_menu.addAction(action_show)
        action_quit = QAction(qtawesome.icon(
            'fa.sign-out', color='white'), f'{language["close"]}', self, triggered=self.app_quit)
        self.icon_menu.addAction(action_quit)
        action_bgon = QAction(qtawesome.icon('fa.toggle-on', color='white'),
                              f'{language["apply_wp"]}', self, triggered=self.apply_background)
        self.icon_menu.addAction(action_bgon)
        action_bgoff = QAction(qtawesome.icon('fa.window-close-o', color='white'),
                               f'{language["restore_wp"]}', self, triggered=self.close_background)
        self.icon_menu.addAction(action_bgoff)
        self.icon_menu.setStyleSheet(
            'QMenu{color:#DDD;background-color:#222;selection-color:'+setting["theme_colour"]+'}')
    ...
    def show_window(self, reason):
        if (reason == QSystemTrayIcon.ActivationReason.Trigger) and not setting['block_home']:
            self.show()
    ...
    def del_background(self):
        ...
        if returnValue == QMessageBox.Cancel:
            ...
            del setting["page_dic"][item.text()]
        self.listwidget.clear()
        for item in setting["page_dic"].keys():
            self.listwidget.addItem(item)
        if setting["page_dic"]:
            setting["page"] = setting["page_dic"][list(
                setting["page_dic"].keys())[0]]["url"]
            setting["start_up"] = setting["page_dic"][list(
                setting["page_dic"].keys())[0]]["exe"]
        else:
            setting["page"] = setting["start_up"] = ""
        save('config', setting)
    ...
    def closeEvent(self, event):
        if not self.really_close:
            self.tray_icon.showMessage(
                language['name'], f'{language["name"]} {language["has_min"]}', QIcon(setting['icon']))
...
class input_window(QWidget):
    ...
    def __init__(self):
        ...
        self.line_name.setStyleSheet(
            'QLineEdit{color:#EEE;background-color:#313131;padding: 7px 15px;}')
        self.line_name.setPlaceholderText(f"{language['tag_name']}")
        self.line_name.setFont(
            QFont(self.fontFamily, setting['font_size'], QFont.Black))
    ...
    def apply_change(self):
        ...
        if not name:
            ...
            msgBox.setIcon(QMessageBox.Warning)
            msgBox.setText(
                f'{language["sure_cover"]}:"{name}"{language["sentting_of"]}?')
            msgBox.setWindowTitle(f"{language['warnning']}")
            msgBox.setStandardButtons(QMessageBox.Ok | QMessageBox.Cancel)
            returnValue = msgBox.exec()
            if returnValue == QMessageBox.Cancel:
        ...
        if os.path.isfile(dic['url']) and not prase.scheme:
            dic['url'] = 'file:///' + \
                str(os.path.abspath(dic['url'])).replace('\\', '/')
        elif not prase.netloc and not prase.scheme:
...
class setting_window(QWidget):
    ...
    def __init__(self):
        ...
        for key in setting.keys():
            if not key in self.opt_zh.keys():
                continue
    ...
    def list_clicked(self, qmodelindex):
        ...
        value, ok = QInputDialog().getText(
            self, f'{language["edit"]}:{key}', f'{language["new_value"]}:')
        self.setStyleSheet(UI_STYLE)
        if not ok:
            msgBox = QMessageBox()
            msgBox.setWindowIcon(QIcon(setting["icon"]))
        ...
        if self.zh_opt[key] in ['zoom', 'alpha']:
            value = float(value)
        elif self.zh_opt[key] in ['font_size', 'width', 'height', 'auto_clear_rate', 'auto_clear_level']:
            value = round(float(value))
        elif self.zh_opt[key] in ['auto_apply', 'show_home', 'guide_reload']:
            value = value == 'True' or value == 'true' or value == '1'
        elif self.zh_opt[key] == "theme_colour" and value[0] != '#':
            value = '#' + value
...
def main():
    ...
    if (setting["clear_storage"] or (if_clear_cache() and setting["auto_clear_level"] >= 3)) and os.path.exists(setting["storage_path"]):
        try:
            for file in tree(setting["storage_path"]):
                os.remove(os.path.join(setting["storage_path"], file))
        except Exception as e:
            log(e)
    ...
    if setting["start_up"]:
        try:
            requests.get(setting['page'], timeout=3)
        except:
            if os.path.isfile(setting["start_up"]):
                win32api.ShellExecute(
                    0, 'open', setting["start_up"].replace("/", "\\"), '', '.\\', 0)
            else:
                pass
    ...
    if if_clear_cache():
        clear_cache(setting["auto_clear_level"])

    main_window = Window()
    if setting['show_home'] and not setting['block_home']:
        main_window.show()
