...
def save(file_name, data):
    with open(f'{filepath}/{file_name}.json', 'w', encoding='utf-8') as g:
        g.write(json.dumps(data, indent=4, ensure_ascii=False))
...
def init():
    ...
    with open(filepath+'config.json', 'r', encoding='utf-8') as g:
        setting.update(json.loads(g.read()))
    save('config', setting)
    ...
    with open(filepath+'language.json', 'r', encoding='utf-8') as g:
        language.update(json.loads(g.read()))
...
def log(data, msg_type: str = 'ERROR'):
    ...
    with open('./log.txt', 'a', encoding='utf-8') as g:
        g.write(
            f'[{time.strftime("%Y-%m-%d_%H:%M:%S",time.localtime(time.time()))}][{msg_type}] {data}\n')
...
def if_clear_cache() -> bool:
    global setting
    if time.time() - setting["last_clear_time"] >= 60*60*setting["auto_clear_rate"]:
        return True
    else:
        return False
