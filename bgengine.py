...
class Background(QWidget):
    ...
    def __init__(self, url=None):
        ...
        qtsize = list(
            map(lambda x: round(x/(rect[2]/window_size[0])), window_size))
        self.resize(*qtsize)
    ...
    def on_click(self, x, y, button, pressed):
        global hwnd_WorkW
        if win32gui.GetForegroundWindow() != hwnd_WorkW:
            return None
        if pressed and str(button) == 'Button.left':
            pos = win32api.MAKELONG(x, y)
            win32api.SendMessage(
                self.winId(), win32con.WM_LBUTTONDOWN, win32con.MK_LBUTTON, pos)
            win32api.SendMessage(
                self.winId(), win32con.WM_LBUTTONUP, None, pos)
    ...
    def main(self):
        ...
        while True:
            ...
            win32gui.EnumChildWindows(
                hwnd_background, lambda hwnd, param: param.append(hwnd), children)
            if not self.control.win_hwnd in children:
                ...
                try:
                    ...
                    log(f"New parent window : {hwnd_background}",
                        msg_type='INFO')
                except Exception as e:
                    log(f"window_hwnd error : {e}")
